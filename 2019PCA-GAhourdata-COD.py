# !/usr/bin/python3
# @File: 2019PCA-GAhourdata-COD.py
# --coding:utf-8--
# @Author:Blazer
# @Time: 2020年08月03日22时17分08秒
# 说明: 继PCA后，通过GA算法优化网络结构得到最优网络结构
...
print("程序开始运行时间为：", strftime("%Y-%m-%d %H:%M:%S", localtime()))
...
# 删除PCA后的数据列
data.drop(['DOS1', 'DOS2', 'DOS3', 'ORPS1', 'ORPS2', 'MLSSS1', 'MLSSS2', 'interS1', 'interS2', 'interS3', 'interS4',
           'DON1', 'DON2', 'DON3', 'ORPN1', 'ORPN2', 'MLSSN', 'interN1', 'interN2', 'interN3', 'interN4', 'Totalwind',
           'DO11', 'DO12', 'DO13', 'ORP11', 'ORP12', 'MLSS1',
           'DO21', 'DO22', 'DO23', 'ORP21', 'ORP22', 'MLSS2',
           'DO31', 'DO32', 'DO33', 'ORP31', 'ORP32', 'MLSS3',
           ...
           'flocc1', 'flocc2', 'flocc3', 'flocc4', 'flocc5', 'flocc6', 'flocc7', 'flocc8',
           'filt1', 'filt2', 'filt3', 'filt4', 'filt5', 'filt6', 'filt7',
           'filt8', 'filt9', 'filt10', 'filt11', 'filt12', 'filt13', 'filt14'], axis=1, inplace=True)

# 增加PCA的主成分
PCAdata = pd.read_excel("E:/pycharm project/小论文/PCAcontroldata.xlsx", names=['index', 'PCA-1', 'PCA-2', 'PCA-3', 'PCA-4'])
data.insert(12, 'PCA-1', PCAdata['PCA-1'])
data.insert(13, 'PCA-2', PCAdata['PCA-2'])
data.insert(14, 'PCA-3', PCAdata['PCA-3'])
data.insert(15, 'PCA-4', PCAdata['PCA-4'])
print(data)
...
def func(x):
    x1, x2, x3, x4 = x
    actfunc = [tf.nn.softmax, tf.nn.softplus, tf.nn.softsign, tf.nn.relu, tf.nn.relu6, tf.nn.tanh, tf.nn.sigmoid]

    def build_model():
        model = keras.Sequential([
            layers.Dense(x1, activation=actfunc[int(x3)], input_shape=[len(train_data.keys())]),
            layers.Dense(x2, activation=actfunc[int(x4)]),
            layers.Dense(1)
        ])  # Watch out keys
...
ga = GA(func=func, n_dim=4, size_pop=200, max_iter=20, lb=[10, 10, 0, 0], ub=[200, 200, 6, 6], precision=[10, 10, 1, 1])
